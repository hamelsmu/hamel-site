...
+     #Force a rebuild to invalidate the cache (you can remove this after the first time)
...
:::{.callout-warning}
### Be Careful When Changing Model Versions
...
Even though distributed inference is interesting for big models that do not fit on a single GPU, exciting things happen when you serve smaller models this way.  Below, I test throughput for `Llama v2 7b` on 1, 2, and 4 GPUs. The throughput is measured by passsing [these 59 prompts](https://github.com/hamelsmu/hamel/blob/master/notes/llm/inference/questions.py) to `llm.generate`.  `llm.generate` is described in the [vLLM documentation](https://vllm.readthedocs.io/en/latest/getting_started/quickstart.html):
...
These are specific observations for `vLLM` and `Llama v2 7b` (the smaller model):
...
- The much larger `Llama v2 70b` model is not much slower than its 7b counterpart!  I was able to get ~400 tok/sec of throughput on 4 A100s which isn't too bad!
