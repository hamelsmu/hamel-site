...
- Torch Serve is an absolute PITA to debug, because its part java and part python.  You can't really use a debugger, and when things fail you might not have the greatest logs.  I am not sure its worth it just for auto-batching, and would rather use FastAPI.  I can't believe I'm saying this but my experience was much smoother with TF Serving despite its hiccups. You have to end up creating custom handlers in many practical scenarios (like using HF models), and at the end of the day you are writing so much code you might as well use FastAPI with K8s.
