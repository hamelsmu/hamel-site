...
Can be referenced with just `http://numbers-api`  -- that is how it's referenced in [the application](https://github.com/sixeyed/kiamol/blob/master/ch03/docker-images/numbers/numbers-web/Dockerfile#L13).  
...
The best way to test internal DNS problems is to use `nslookup` in a pod.  You can deploy a pod specifically for network testing from the [official k8s docs](https://kubernetes.io/docs/tasks/administer-cluster/dns-debugging-resolution/):
...
If you are experiencing further issues, follow [these debugging instructions](https://kubernetes.io/docs/tasks/administer-cluster/dns-debugging-resolution/)
