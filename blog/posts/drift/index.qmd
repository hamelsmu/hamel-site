...
description: Quickly detect a large class of bugs using an automated technique.
...
No matter how careful you are, bugs can still slip through the cracks. A high-value activity is to audit all your existing AI/ML projects for drift.
...
6.  If the classifier demonstrates sufficient predictive power (ex: AUC >=0.60) then we know there is drift.
...
This demo is from a real-world example where an unintentional change in a prompt template caused unexpected behavior in a model.  We were able to apply the tool and quickly find the root cause of the issue. The modeling code is embarrassingly simple and located at [`ft_drift/model.py`](https://github.com/hamelsmu/ft-drift/blob/main/ft_drift/model.py).  The point is you don't need sophisticated techniques to get started.
...
[^1]: I first of this technique in 2016 from [this blog post](https://fastml.com/adversarial-validation-part-two/) by Zygmunt Zając.  Throughout the years, I've seen this technique used in a variety of contexts, sometimes with different names.
