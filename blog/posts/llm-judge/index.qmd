...
![Tracking a bunch of scores on a 1-5 scale is often a sign of a bad eval process, especially when starting out.](blog_header.png)

Earlier this year, I wrote [why your AI product needs evals](https://hamel.dev/blog/posts/evals/). The response was amazing! Many of you asked, "How do I get started with LLM-as-a-judge?" This guide shares what I've learned after helping over [30 companies](https://parlance-labs.com/) set up their evaluation systems.

## The Problem: AI Teams Are Drowning in Data

Ever spend weeks building an AI system, only to realize you have no idea if it's actually working? You're not alone.

When teams start using Language Models (LLMs) to evaluate AI outputs, they often make these mistakes:

1. **Too Many Metrics**: Creating numerous measurements that become unmanageable.
2. **Complex Scoring Systems**: Using scales like 1-5 across multiple dimensions.
...
The solution is to use a technique that I call **"Critique Shadowing"**. Here's how to do it, step by step.

## Step 1: Find _The_ Principal Domain Expert

Here's a secret most AI teams miss: You don't need to make everyone happy. You just need to convince one key person.

In most organizations there is usually one (maybe two) key individuals whose judgment is crucial for the success of your AI product. These are the people who have the final authority or whom you must convince that the AI is ready for deployment. Identifying and involving this **Principal Domain Expert** early in the process is critical.

**Why is finding the right domain expert so important?**

- **They Set the Standard**: This person defines what is acceptable or not for your AI's performance. Their opinion often carries the most weight in decision-making.
...
You want to define dimensions that make sense for your use case.  For example, here are ones that I often use for many B2C applications:

1. **Features**: Specific functionalities of your AI product.
2. **Scenarios**: Situations or contexts the AI may encounter and needs to handle.
...
|-----------------------------|---------------------------------------------------------------------------------|
| **Multiple Matches Found**  | User's request yields multiple results; AI must help narrow down options.       |
| **No Matches Found**        | User's request yields no results; AI should suggest alternatives or corrections.|
| **Ambiguous Request**       | User input is unclear; AI needs to ask clarifying questions.                    |
| **Invalid Data Provided**   | User provides data not present in the system; AI must handle gracefully.        |
| **System Errors**           | Technical issues occur; AI should inform the user appropriately.                |
| **Incomplete Information**  | User omits necessary details; AI must guide the user to provide missing info.   |
| **Unsupported Feature**     | User requests an unsupported feature; AI must inform the user politely.         |
...
Often, you'll do a combination of both to ensure comprehensive coverage.  

This post discusses how to generate synthetic data since leveraging existing data is much more straightforward.  Just remember that you want good coverage across the dimensions you've defined.
...
Alongside a binary pass/fail judgment, it's important to write a detailed critique.  These critiques:
...
A common mistake is straying from binary pass/fail judgments.  I constantly see evaluation dashboards that look like this:

![Don't do this.  This is illustrative, but representative of many eval dashboards I've seen in the wild](dashboard.png)
...
Finally, you need to remove all friction from removing data. I've written about this [here](../../../notes/llm/finetuning/04_data_cleaning.qmd).  Sometimes, you can just use a spreadsheet.  It's a judgement call in terms of what is easiest for the domain expert. I found that I often have to provide additional context to help the domain expert understand the user interaction, such as:
...
The number of examples you need depends on the complexity of the task.  My heuristic is that I start with around 30 examples each time I iterate.  From there, I keep going until I'm not learning anything new.

Next, we'll look at how to use this data to build an LLM judge.

## Step 4: Fix Errors

After looking at the data, it's likely you will find errors in your AI system.  For example, one of my clients found that their router for agents was always fetching the wrong agent after critiquing ~20 examples.  Instead of plowing ahead and building an LLM judge, you want to fix any obvious errors.  Remember, the whole point of the LLM as a judge is to help you find these errors, so it's totally fine if you find them earlier!  
...
I usually adjust the prompts by hand.  I haven't had much luck with prompt optimizers like DSPy.  However, my friend [Eugene Yan](https://eugeneyan.com/) has just released a promising tool named [ALIGN Eval](https://eugeneyan.com/writing/aligneval/).  I like it because it's simple and effective.  Also, don't forget the approach of [continual in-context learning](https://blog.langchain.dev/dosu-langsmith-no-prompt-eng/) mentioned earlier - it can be very effective when implemented correctly.

### The Human Side of the Process

Something unexpected happened during this process. Phillip, our domain expert at Honeycomb, found that reviewing the LLM's critiques helped him articulate his own evaluation criteria more clearly. He told me, "Seeing how the LLM breaks down its reasoning made me realize I wasn't being consistent about how I judged certain edge cases." This is a pattern I've seen repeatedly—the process of building an LLM judge often helps standardize evaluation criteria.
...
- The expecatations of alignment are unrealistic or not feasible.
...
I prefer not to fine-tune LLM judges.  I'd rather spend the effort fine-tuning the actual LLM instead.  For example, you can leverage a LLM judge to curate and transform data for fine-tuning your primary model.  For example, you can use the judge to:
...
Sometimes, you may encounter difficulties with fitting everything you need into the prompt, and may have to get creative about how you structure the examples.  However, this is becoming less of an issue with expanding context windows and [prompt caching](https://platform.openai.com/docs/guides/prompt-caching).
...
1. You are an indpendent developer who is also a domain expert.
...
### If I have a good judge LLM, isn’t that also the LLM I’d use for my use case?

If the cost of the most powerful LLM is not prohibitive, and latency is not an issue, then the utility of the judge LLM as described in this post is significantly diminished.  In this case, it probably makes sense to weight your effort towards fixing the errors you find in the primary model, rather than building a judge. If latency is not an issue, you can even have your primary model write internal critiques and retries (chain-of-thought, etc) before responding.

However, in this case specialist LLM judges can still be useful.  But you should go through the process of looking at data and critiquing the LLM's output before you adopt specialized judges.
...
### Should I focus on prompting or fine-tuning for optimizing my judge?

I recommend focusing on prompting.  I would only fine-tune in rare cases.  It's usually not worth the effort of fine-tuning a judge relative to improving your primary model.  However, fine-tuning guardrails or other specialized judges can be useful (especially if they are small classifiers).

However, as I mentioned earlier in this post, I like to use LLM judge to help me curate a dataset that I can use for distillation into a smaller model.
...
These are some of the resources I recommend to learn more about LLM judges:

- [Who Validates the Validators? Aligning LLM-Assisted Evaluation of LLM Outputs with Human Preferences](https://arxiv.org/abs/2404.12272): This paper provides a good overview of the challenges of evaluating LLMs, and the importance of following a good process.
- [Align Eval](https://humanloop.com/blog/align-eval-a-new-approach-to-evaluating-llms): Eugene Yan's new tool that helps you build LLM judges by following a good process.
...
- [Enhancing LLM-As-A-Judge with Grading Notes](https://www.databricks.com/blog/enhancing-llm-as-a-judge-with-grading-notes): This is _very_ similar to the approach articulated in this blog post, and provides another point of view regarding the utility of writing critiques (they call them grading notes).
- [Custom LLM as a Judge Cookbook from OpenAI](https://cookbook.openai.com/examples/custom-llm-as-a-judge) by Ankur Goyal and Shaymal Anadkt provide an end-to-end example of building a LLM judge, and for the use case highlighted, authors found that a classification approach was more reliable than numeric ratings (consistent with this blog post).
