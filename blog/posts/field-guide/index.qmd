...
This scene has played out dozens of times over my two years of independent AI consulting. Teams invest weeks architecting complex AI systems, but can't tell me if their changes are helping or hurting.
...
- [Why focusing on tools over process is killing your AI projects](#the-most-common-mistake-focus-on-tools-over-process)
- [How error analysis consistently reveals the highest-ROI improvements](#everyone-skips-error-analysis-and-its-the-highest-roi-activity)
- [Why a simple data viewer is your most important AI investment](#the-most-important-ai-investment-a-simple-data-viewer)
- [How to empower domain experts (not just engineers) to improve your AI](#empower-domain-experts-to-write-prompts)
- [Why synthetic data is more effective than you think](#bootstrapping-your-ai-with-synthetic-data-is-effective-even-with-zero-users)
- [How to maintain trust in your evaluation system](#maintaining-trust-in-evals-is-critical)
- [Why your AI roadmap should count experiments, not features](#your-ai-roadmap-should-count-experiments-not-features)

I'll anchor everything in real examples. While every situation is unique, you'll see patterns that apply regardless of your domain or team size.

Let's start by examining the most common mistake I see teams make – one that derails AI projects before they even begin.

## The Most Common Mistake: Focus on Tools Over Process
...
One client proudly showed me their evaluation dashboard, which looked like this:
...
This is what I call the "tools trap" – the belief that adopting the right tools or frameworks (in this case, eval tools and generic metrics) will solve your AI problems. It's particularly dangerous because it feels like progress. You're building things, deploying features, staying current with the latest tech. But without meaningful measurement, you're essentially flying blind.

These generic metrics are worse than useless – they actively impede progress in three ways:

First, they create a false sense of measurement. Teams think they're data-driven because they have dashboards, but they're tracking vanity metrics that don't correlate with actual user problems.

Second, generic metrics fragment your attention. Instead of focusing on the few metrics that matter for your specific use case, you're trying to optimize multiple dimensions simultaneously. When everything is important, nothing is.

Third, and most dangerously, these metrics create the illusion of progress. I've seen teams celebrate improving their "helpfulness score" by 10% while their actual users were still struggling with basic tasks. It's like optimizing your website's load time while your checkout process is broken – you're getting better at the wrong thing.

What's the alternative? Let me share a story from [Nurture Boss](https://nurtureboss.io/), a company building AI assistants for the apartment industry.  I've embedded a video [below] where Jacob walks through his journey, but here is a synopsis:

When I first started working with them, their founder Jacob was struggling to understand if their AI was actually working.

> "When we released our products, it was a giant black box for us in terms of what was happening.  We didn't have a way to know if it was going well or not systematically."

I gave Jacob what seemed like simple advice: look at your data. What's particularly striking is his initial skepticism:

> "I had no idea or understood at all the value that would come from quite literally just looking at our data. Hamel, I don't know if I ever expressed this with you, but it was kinda frustrating when you kept telling me that. Like, what is the secret here that I'm gonna learn? And then I got into it, and realized how impactful, insightful, and important it was that we do that, and we'll continue to do that."

The approach was straightforward: they built a simple data viewer that let the team examine conversations between their AI and users (more on that later). They could see exactly what users were asking for, how the AI responded, and what tools it used. More importantly, they could quickly flag issues and add notes explaining what went wrong.

The results were transformative. As Jacob explains:

> "Being able to actually see what people are asking your product, every day, and identifying gaps where they're looking to get problems solved that your product does not know how to solve is invaluable. It builds your product road map for you...

Let me give you a concrete example. Through this process, they discovered their AI was struggling with date handling – a seemingly simple task that was failing 33% of the time. Users would say "let's schedule a tour two weeks from now" and the AI would suggest random dates, sometimes even in past years. Instead of immediately reaching for new tools, they:
...
4. Measured improvement on these concrete metrics
...
This pattern repeats across successful AI teams: they prioritize measurement and iteration over tool selection. They build simple systems to track specific, meaningful metrics. And most importantly, they look at their data. 

I recommend watching Jacob explain this journey himself:

{{< video https://youtu.be/bHYKpmD7460?si=Y2YfvvWUw5L2JD1A width="600" height="300" >}}

Which brings us to the highest ROI activity in AI development – one that almost everyone skips...


## Everyone Skips Error Analysis (And It's The Highest ROI Activity)

Error analysis sounds boring. It's not as exciting as trying the latest LLM or implementing a new agent framework. **But it's the single most valuable activity in AI development**, and I've never seen a team regret investing in it. Let me show you how this played out at NurtureBoss.

Remember how Jacob's team was struggling with their apartment leasing AI? To expand on our error analysis process: we started with nothing more than a spreadsheet and an LLM. For each failed interaction, we wrote brief notes about what went wrong, then used the LLM to classify these notes into categories. This bottom-up approach let patterns emerge naturally from the data, rather than trying to pre-define what we were looking for.

We then summarized the error categories with a pivot table (truncated for confidentiality):

![Excel Pivot Tables are a simple tool, but they work!](images/pivot.png){width="200"}

Three issues accounted for over 60% of their problems:
- Conversation flow issues: The AI would miss context or respond awkardly to user queries or go off topic.
...
The classification approach is key. Instead of trying to pre-define error categories, we let them emerge from the data. We wrote natural language descriptions of what went wrong in each case, then used an LLM to group similar issues. This revealed patterns we wouldn't have thought to look for upfront.
...
But there's a deeper skill at play here – what I call a "nose for data." It's a combination of skepticism and curiosity that leads you to ask questions like:

- Why did this metric suddenly improve?
- What are we not measuring?
- Does this number match our qualitative experience?

Let me share a striking example from a recruiting platform I worked with. Their AI was generating initial outreach emails to candidates, and they were tracking success using edit distance – measuring how much recruiters modified the AI's output. When this metric started showing improvement, the team was ecstatic. The average edit distance had dropped by 40%!

But something felt off. When we dug deeper, we discovered a crucial detail: most of their recruiters were non-native English speakers. These recruiters were actively making edits that introduced grammatical errors and awkward phrasing into otherwise correct AI-generated text. In fact, when we showed the emails to hiring managers, they rated the less-edited versions as significantly better. The metric had improved (lower edit distance) because the AI was actually doing a better job than the human editors, but the team's reliance on edit distance as a metric led them to incorrectly conclude their recruiters should keep making changes.

There's no decision tree for data analysis. You can't follow a rigid process and expect insights to emerge. Instead, you need to develop intuition through experience. The good news is that this intuition builds quickly when you make looking at data a habit.

As Jacob from NurtureBoss put it:

> "The most valuable thing we did was create a habit of looking at our data every single day. It completely changed how we thought about our product."
...
The power of these interfaces isn't in their sophistication. It's that they remove friction from looking at real data. When domain experts can easily examine AI outputs, they consistently find insights that surprise the technical team.

Here's what makes a good data viewer:
...
3. Enable quick filtering and sorting. Teams need to easily dive into specific error types.
4. Have hotkeys that allow users to navigate quickly and annotate without clicking.

It doesn't matter what web frameworks you use - use whatever you are familiar with.  Because I'm a python developer, my current favorite web framework is [FastHTML](https://fastht.ml/docs/) coupled with [MonsterUI](https://www.answer.ai/posts/2025-01-15-monsterui.html), because it allows me to define the back-end and front-end code in one small python file. The [FastHTML gallery](https://fastht.ml/gallery/) has an example of an annotation app in the "Applications" section.  

You don't need to build something complex. Even a spreadsheet can work initially. The key is making it easy for domain experts to spot patterns. This brings us to another counter-intuitive lesson: the people best positioned to improve your AI system are often the ones who know the least about AI.

## Empower Domain Experts To Write Prompts

I recently worked with an AI startup building an interactive learning platform. Their product helps students master complex concepts through guided problem-solving – the AI adapts in real-time, providing personalized scaffolding and feedback as students work through challenges. It's the kind of product that requires deep expertise in both education and technology.

Like many teams, they followed what seemed like a logical process: their product manager – a learning design expert, would create detailed PowerPoint decks explaining pedagogical principles, example dialogues, and desired learning outcomes. She'd present these to the engineering team, who would then work to translate her expertise into prompts.
...
Of course, there's nuance here. Technical terminology exists for a reason – it provides precision when talking with other technical stakeholders. The key is adapting your language to your audience:

- With non-technical domain experts: Focus on capabilities and outcomes, not implementation details
- In mixed audiences: Introduce technical terms with plain language explanations 
- With technical teams: Use precise terminology when needed for implementation discussions

The most effective teams I've seen create a shared vocabulary that bridges these worlds. They don't abandon technical terms entirely – they just make sure everyone understands what they mean in practice.
...
As I wrote in my [LLM-as-a-Judge blog post](https://hamel.dev/blog/posts/llm-judge/#generating-data), synthetic data can be remarkably effective for evaluation. [Bryan Bischof](https://www.linkedin.com/in/bryan-bischof/), Former Head of AI at Hex, put it perfectly:
...
Here's an example of how we might use these dimensions with real data to generate test cases for the property search feature (this is just psuedo code, and very illustrative):
...
The simplicity of binary decisions also makes it easier to align automated evaluations with human judgment. When measuring agreement between humans and LLMs, calculating agreement on binary decisions is straightforward. This simplicity helps teams maintain trust evaluation systems.
...
![Agreement rates between LLM evaluator and human expert. Yes, we are using an a spreadsheet again - but it works!](images/score.png)
...
This wasn't wasted time – it was the critical foundation that enabled everything that followed. Once that evaluation framework was in place, the pace of improvement accelerated dramatically. The team could run thousands of experiments, quickly identify which approaches worked best, and iterate rapidly. They could confidently say "this change improved completion quality by X%" rather than relying on gut feelings or anecdotal feedback.

This pattern repeats across successful AI teams. The upfront investment in evaluation infrastructure feels slow initially, but it creates a foundation for rapid experimentation and improvement later. Without trusted evaluation metrics, teams get stuck in endless debates about whether changes are actually helping or hurting.
...
> "I try to reassure leadership with timeboxes. At the end of three months, if it works out, then we move it to production. At any step of the way, if it doesn't work out, we abort."
...
This level of transparency about both successes and failures builds trust with stakeholders, even when the outcomes aren't what they hoped for.
...
> "For the first two to three months, it was all massive failures. [...] And then [a breakthrough] came out. [...] Within a month, that problem was solved. So you can see that in the first quarter or even four months, it was going nowhere. [...] But then you can also see that all of a sudden, some new technology comes along, some new paradigm, some new reframing comes along that just [solves] 80% of [the problem]."
...
- **[Rapidly Improve AI Products With Evals](https://bit.ly/evals-ai)**, a course I'm teaching with Shreya Shankar. The course provides hands-on experience with techniques such as error analysis, synthetic data generation, and building trustworthy evaluation systems. It includes practical exercises and personalized instruction through office hours.
