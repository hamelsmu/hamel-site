...
[Dokku](https://dokku.com/) is an open-source Platform as a Service (PaaS) that runs on a single server of your choice. **It's like [Heroku](https://www.heroku.com/), but you own it.** It is a great way to get many of the benefits of Heroku without the costs (Heroku can get quite expensive!).

**I run my Dokku server on a $7/month vps on [OVHcloud](https://us.ovhcloud.com/).**  I serve multiple websites and applications on this server.  Some of the features I love about Dokku:
...
I have deployed various applications [for my clients](https://nbsanity.com/), which include things like [nbsanity](https://nbsanity.com/), [data cleaning tools for LLMs](https://langfree.parlance-labs.com/tutorials/shiny.html#run-the-shiny-app-locally), and private static sites.  An easy way to do this is with a Docker container.
...
# this directory contains app.py that defines a FastApi app
WORKDIR /app/
...
That's it - your app should be running on the Dokku host!  You can scale it up/down with the following command (and do a whole lot more):
...
Then do the following from the root of your git repo that contains the static site:
...
3. add the dokku host as a remote
...
## Deploying Dokku Apps with GitHub Actions

You can automatically deploy Dokuu apps with GitHub Actions, which is helpful if you don't want to fiddle with pushing to the Dokku host.  Here is an example GitHub Action workflow that does this:
...
I needed notes for myself, as I had to dig up these details whenever I wanted to deploy a new app.  I hope you find it useful, too!
