...
Jason Liu's ["There Are Only 6 RAG Evals"](https://jxnl.co/writing/2025/05/19/there-are-only-6-rag-evals/) provides a framework that maps well to this separation. His Tier 1 covers traditional IR metrics for retrieval that don't require LLMs. Tiers 2 and 3 evaluate relationships between Question, Context, and Answer—like whether the context is relevant (C|Q), whether the answer is faithful to context (A|C), and whether the answer addresses the question (A|Q).

When implementing Liu's Tier 2 and 3 metrics, you can't just use prompts off the shelf. Follow the standard LLM-as-judge process: create labeled examples, measure your judge's True Positive and True Negative rates against human labels, and optionally apply the [Rogan-Gladen correction](https://influentialpoints.com/Training/estimating_true_prevalence.html) when estimating failure rates in production. Skip this validation and your judges may not reflect your actual quality criteria.
