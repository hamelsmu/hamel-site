...
This post curates the most common questions Shreya and I have recieved from [teaching 700+](https://bit.ly/evals-ai){target="_blank"} engineers & PMs in AI Evals.  _Warning: These are sharp opinions about what works in most cases.  They are not universal truths. Use your judgment._
...
Generic metrics like BERTScore, ROUGE, cosine similarity, etc. are generally not useful for evaluating LLM outputs. Instead, we recommend using [error analysis](#q-why-is-error-analysis-so-important-in-llm-evals-and-how-is-it-performed) to design metrics specific to your application's behavior. These task-specific metrics are often a [binary pass/fail](#q-why-do-you-recommend-binary-(passfail-evaluations-instead-of-1-5-ratings-(likert-scales)).) metric or a code- based assertion.

As an example, consider a real estate CRM assistant. Suggesting showings that aren't available or confusing client personas is problematic. Generic metrics like similarity or verbosity won't catch this. A relevant quote from the course:
