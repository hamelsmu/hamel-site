...
Here is a FAQ you have drafted for me.  Can you look into objections I might get about this?
...
<!-- ## Q: Should you write evals anytime you use a LLM for the same task?

No. You need to assess when the cost of writing evals is worth the benefit. Here are some examples where I do NOT use evals (becaause their benefit is low):

1. **When I am the sole customer.** For my own personal workflows, like writing or development assistants, I can immediately spot and correct errors. The feedback loop is instant and the only person affected is me, so a formal evaluation process is unnecessary.
2. **Ad-hoc tasks that aren't repeated.** If you cannot amoritize the cost of writing evals over many uses (from either yourself or across users), their ROI is limited.
3. **When I'm prototyping an idea.** There is no sense writing an eval before you have a product. Introducing them too early adds unnecessary friction.

The trigger for introducing evals is when a prototype evolves into a product that serves others, has sufficient usage or high cost of failure (ex: user churn, reputational damage, etc.). There is no one-size-fits-all checklist: you must assess the cost-benefit of evals for your specific use case.  For example, if you are ready to scale to 100s of users, it makes sense to proactively write evals (perhaps with [synthetic data](#q-what-is-the-best-approach-for-generating-synthetic-data)). -->
